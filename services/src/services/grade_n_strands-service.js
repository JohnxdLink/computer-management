...
} = require("../database/queries/grade-n-strands-query.js");

// CREATE
const createGradeNStrand = async (
  grade_level,
  strand,
  description
) => {
  ...
  const [result] = await db.query(CREATE_GRADE_N_STRAND, [
    grade_level,
    strand,
    description,
  ]);
...
const findGradeNStrand = async (grade_level, strand) => {
  ...
  const [rows] = await db.query(FIND_GRADE_N_STRAND, [
    grade_level,
    strand,
  ]);
...
const updateGradeNStrand = async (
  ...
  const [result] = await db.query(UPDATE_GRADE_N_STRAND, [
    grade_level,
    strand,
    description,
    id,
