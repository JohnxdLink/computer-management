...
const {
  ...
  UPDATE_ISSUE_REPORT,
  DELETE_ISSUE_REPORT,
} = require("../database/queries/issue_reports-query.js");

// CREATE
const createIssueReport = async (computer_id, reported_by, issue_description, priority, status, resolved_date) => {
  if (!computer_id || !reported_by || !issue_description || !priority || !status) {
    throw new Error("Computer ID, reported by, issue description, priority, and status are required.");
  }
...
const findIssueReportsByComputerId = async (computer_id) => {
  ...
  const [rows] = await db.query(FIND_ISSUE_REPORTS_BY_COMPUTER_ID, [computer_id]);

  return rows;
};

// UPDATE
const updateIssueReport = async (id, computer_id, reported_by, issue_description, priority, status, resolved_date) => {
  if (!id) {
    throw new Error("Issue report ID is required.");
  }
